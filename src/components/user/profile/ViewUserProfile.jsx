...
const ViewUserProfile = () => {
    ...
    const fetchProfileData = async () => {
      try {
        ...
        const response = await fetch(
          `http://localhost:5000/api/cso/res/${registrationId}`
        );
  ...
  const handleFileSelection = (event) => {
    ...
    if (
      selectedValue === profileData.tin_certificate ||
      selectedValue === profileData.registration_certificate
    ) {
          ...
          <div className="bg-gradient-to-r from-gray-800 to-indigo-600 p-8 text-center">
            <div className="relative inline-block group">
              <img
                src={
                  profileData.logo
                    ? `${process.env.REACT_APP_API_URL}/${profileData.logo}`
                    : "http://localhost:5000/logos/person.png"
                }
                alt="Profile"
              ...
              <div className="space-y-6">
                ...
                {selectedFile && (
                  <div className="border-2 border-dashed border-gray-200 rounded-2xl overflow-hidden transition-all duration-300">
                    {renderFile(`${API_BASE_URL}/${selectedFile}`)}
                    <div className="bg-gray-50 p-4 border-t border-gray-100">
                      <a
                        href={`${API_BASE_URL}/${selectedFile}`}
                        download
                        target="_blank"
